...
#########################################################
# Percona Test Bench - rqg_prepare                      #
#########################################################
...
function prepare()
{
	...
	ptb_sql $PTB_OPT_server_id "DROP DATABASE IF EXISTS rqg1"
	rc=$?
	if [ $rc -ne 0 ]; then
	...
	ptb_sql $PTB_OPT_server_id "CREATE DATABASE rqg1"
	rc=$?
	if [ $rc -ne 0 ]; then
		ptb_report_error "$rpt_prefix - ptb_sql failed with $rc."
		ptb_cleanup 0
...
PTB_OPTION_DESCRIPTORS=(\
	...
	"prepare-rootdir OPT 1 STR Directory where RQG binaries are located." \
